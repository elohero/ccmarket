...
local function agoText(ts)
    local d = os.time() - ts
    if d < 60 then return u8('������ ���') end
    if d < 3600 then return math.floor(d / 60) .. u8(' ���') end
    if d < 86400 then return math.floor(d / 3600) .. u8(' �') end
    return math.floor(d / 86400) .. u8(' �')
end
...
local function loadTradesFromCsv()
    ...
    dataVersion = dataVersion + 1
    print(('[CC] ��������� ������: %d (��������� �� ��������� ��������: %d)'):format(loaded, skipped))
end
...
local PUSH_EVERY   = 180   -- �������: �������� ������������ + ��������� ������
local OPEN_EVERY   = 120   -- ���� �������: ����������� ������
local IDLE_EVERY   = 300   -- ���� ������� � �� �� �������: ������ �������� ������
...
local function detailPanel(stats)
    ...
    colored(GREY, serverName(stats.srv) .. u8('   ������ ') .. cur .. u8('   ID ') .. stats.id)
    colored(GREY, u8('������: ') .. stats.count .. u8('   ����: ') .. formatMoney(stats.qty))

    imgui.Separator()
    imgui.Text(u8('�������: ') .. formatMoney(stats.median) .. ' ' .. cur)
    colored(GREY, u8('��� ') .. formatMoney(stats.min) .. u8('   ���� ') .. formatMoney(stats.max))
    ...
    if best then
        imgui.Separator()
        colored(GREY, u8('������ �� �����'))
        if best.buyMax then imgui.Text(u8('���� ��:  ') .. formatMoney(best.buyMax) .. ' ' .. cur) end
        if best.sellMin then imgui.Text(u8('����� ��: ') .. formatMoney(best.sellMin) .. ' ' .. cur) end
    end
...
imgui.OnFrame(
    ...
    function(this)
        ...
        do
            ...
            if active then imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.30, 0.45, 0.65, 1.00)) end
            if imgui.Button(u8('���') .. '##srvall', imgui.ImVec2(50, 0)) then
        ...
        for _, n in ipairs(tracked) do
            ...
            if imgui.IsItemHovered() then
                local st = srvState[n]
                imgui.BeginTooltip()
                imgui.Text(serverName(n) .. u8('  (����� ') .. n .. ')')
                imgui.Text(u8('������: ') .. currencyOf(n))
                imgui.Text(u8('������: ') .. st.snapshots)
                if st.lastOkTs then
                    imgui.Text(u8('��������� ����: ') .. agoText(st.lastOkTs))
                else
                    imgui.Text(u8('������ ���� �� ����'))
                end
                if st.apiStatus ~= '' then
                    imgui.Text(u8('�����: ') .. st.apiStatus)
                end
                if syncActive then
                    if st.isHolder then
                        imgui.Text(u8('����: ������� �'))
                    else
                        imgui.Text(u8('����: �������� ') .. tostring(st.leaseHolder or '?'))
                    end
                end
                imgui.EndTooltip()
            end
        ...
        do
            ...
            if stale then
                imgui.SameLine(0, 15)
                colored(ORANGE, u8('! ') .. stale.name .. u8(': ��� ������ ') .. agoText(os.time() - stale.age))
            end
        end
        ...
        headerCell(COL_NAME, '�������', 2)
        imgui.SameLine(); headerCell(COL_SRV, '������', 2)
        imgui.SameLine(); headerCell(COL_WHERE, '���', 2)
        imgui.SameLine(); headerCell(COL_LAST, '���������', 0)
        imgui.SameLine(); headerCell(COL_MIN, '���', 3)
        imgui.SameLine(); headerCell(COL_MED, '�������', 3)
        imgui.SameLine(); headerCell(COL_MAX, '����', 3)
        imgui.SameLine(); headerCell(COL_CNT, '������', 1)
        imgui.SameLine(); headerCell(COL_QTY, '����', 4)
        imgui.SameLine(); headerCell(COL_WHEN, '�����', 0)
        imgui.Separator()
        ...
        for i = 1, #rowsCache do
            ...
            colored(GREY, row.side == 'sell' and u8('�����') or u8('����'))
...
function main()
    ...
    sampRegisterChatCommand('ccreload', function()
        loadItemsIndex()
        sampAddChatMessage('[CC] ���������� ��������� ������������', 0x66CCFF)
    end)

    sampRegisterChatCommand('ccpath', function()
        sampAddChatMessage('[CC] �������: ' .. csvPath, 0x66CCFF)
    end)

    sampRegisterChatCommand('ccadd', function(arg)
        local n = tonumber(arg)
        if not n or n <= 0 then
            sampAddChatMessage('[CC] ����� ����� �������, ��������: /ccadd 201', 0xFF9955)
            return
        end
        if trackedSet[n] then
            sampAddChatMessage('[CC] ���� ������ ��� �������������', 0xFF9955)
            return
        end
        addServer(n)
        markDirty()
        sampAddChatMessage('[CC] �������� ������ ' .. n, 0x66CCFF)
    end)

    sampRegisterChatCommand('cclist', function()
        for _, n in ipairs(tracked) do
            local st = srvState[n]
            sampAddChatMessage(('[CC] %d - ������ %d %s'):format(
    ...
    sampAddChatMessage(('[CC] ���� �������, ��������: %d. ����: /cc'):format(#tracked), 0x66CCFF)
